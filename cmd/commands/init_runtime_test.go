...
func TestWriteTopology_PopulatesManagedURLs(t *testing.T) {
	...
	assert.Equal(t, "https://demo-worker1.ns.svc:8490", got.Peers[0].URL)
	assert.Equal(t, "worker2", got.Peers[1].Name)
	assert.Equal(t, "https://demo-worker2.ns.svc:8490", got.Peers[1].URL)
}
...
func TestWriteTopology_PreservesExternalURLs(t *testing.T) {
	...
	require.Len(t, got.Peers, 2)
	assert.Equal(t, "https://demo-inside.ns.svc:8490", got.Peers[0].URL)
