...
import (
	...
	"github.com/kynoproj/kynomesh/pkg/client/clientset/versioned/typed/kynomesh/v1alpha1"
...
func GetValidator(ctx context.Context, kynomeshClient v1alpha1.KynomeshV1alpha1Interface, kind metav1.GroupVersionKind, oldBytes []byte, newBytes []byte) (Validator, error) {
	log := logging.FromContext(ctx)
	switch kind.Kind {
	case kmv1.AgentSetGroupVersionKind.Kind:
		var newSpec *kmv1.AgentSet
		if len(newBytes) > 0 {
			newSpec = &kmv1.AgentSet{}
			if err := json.Unmarshal(newBytes, newSpec); err != nil {
				log.Errorf("Could not unmarshal new raw object: %v", err)
				return nil, err
			}
		}
		var oldSpec *kmv1.AgentSet
		if len(oldBytes) > 0 {
			oldSpec = &kmv1.AgentSet{}
			if err := json.Unmarshal(oldBytes, oldSpec); err != nil {
				log.Errorf("Could not unmarshal old raw object: %v", err)
				return nil, err
			}
		}
		return NewAgentSetValidator(oldSpec, newSpec), nil
	default:
