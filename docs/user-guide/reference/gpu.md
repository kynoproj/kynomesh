...
Your cluster must support GPU scheduling and have the appropriate device plugin
installed. See the
...
> **Important:** for GPUs, Kubernetes requires that `requests` and `limits` be
> the same (or that you specify only `limits`).
